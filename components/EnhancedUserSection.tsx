...
const EnhancedUserSectionFull = () => {
  const { user, updateUser } = useUser(); // Using your existing hook
  ...
  const getRankBgColor = (rank?: string) => {
    ...
    if (lowerRank.includes('pupil')) return 'bg-green-500';
    return 'bg-gray-500';
  };
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      await updateUser(validation.userData);
      setHandle('');
      setValidation({
        isValid: false,
        isValidating: false,
  ...
  if (user) {
    return (
      <Card className="w-full max-w-4xl"> {/* Made it wider than your original max-w-2xl */}
  ...
  // Enhanced username input form (this replaces your Settings component)
  return (
    <Card className="w-full max-w-4xl"> {/* Made it wider than your original max-w-2xl */}
